...
export function AppShell({ children }: { children: ReactNode }) {
  const { role, setRole, selfId, setSelfId } = useAppContext();
  const pathname = usePathname();
  const needsName = role === "Developer";

  return (
    <Box sx={{ minHeight: "100%" }}>
      <AppBar position="fixed" sx={{ zIndex: (t) => t.zIndex.drawer + 1, borderBottom: "1px solid", borderColor: "divider" }} elevation={0}>
        <Toolbar sx={{ gap: 3 }}>
          <LogoLockup />
