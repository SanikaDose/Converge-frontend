 ...
 */
import { TEMPLATE, EMPLOYEES, genId } from "./data.js";
import { addWorkingDays, businessDaysBetween, todayISO } from "./dateUtils.js";
...
export function computePlanned(startDate, dayOffset, duration) {
...
export function buildTasks(startDate, phases) {
  const tasks = [];
...
export function suggestedEndDate(startDate) {
...
------------------------------------------------------------------------ */
export function ensureProjectShape(detail) {
  ...
  const tasks = (detail.tasks || []).map(t => {
    const phaseId = t.phaseId || (phases[t.phaseIndex] ? phases[t.phaseIndex].id : phases[0]?.id);
    return {
      order: 0,
      description: "",
      assignedTo: null,
...
export function guessEmployeeIdFromFreeText(name) {
  if (!name) return null;
...
export function overdueWorkingDays(task, today) {
...
export function phaseSummaries(phases, tasks, today, projectStartDate) {
  return phases.slice().sort((a, b) => a.order - b.order).map((phase) => {
    ...
    return {
      ...
      weekStart: phaseStart ? Math.floor((new Date(phaseStart) - new Date(projectStartDate)) / 86400000 / 7) + 1 : null,
      weekEnd: phaseEnd ? Math.floor((new Date(phaseEnd) - new Date(projectStartDate)) / 86400000 / 7) + 1 : null,
    };
  });
}
...
export function projectStatusFromPhases(phaseRows) {
...
export function computeAchievement(task) {
...
export function requestScheduleChange(task, changes, actor, reason) {
  return {
    ...task,
    pendingChange: {
      id: genId("chg"),
...
export function approveScheduleChange(task, approver) {
  ...
  Object.entries(changes).forEach(([field, to]) => {
    history.push({
      ts: new Date().toISOString(), field: fieldLabel(field), from: task[field], to,
...
export function rejectScheduleChange(task, approver, comment) {
  if (!task.pendingChange) return task;
  const history = [...(task.history || []), {
...
------------------------------------------------------------------------ */
export function toTaskLite(tasks) {
  return tasks.map(t => ({ phaseId: t.phaseId, plannedFinish: t.plannedFinish, status: t.status }));
}
export function toPhasesLite(phases) {
  return phases.map(p => ({ id: p.id, critical: p.critical, name: p.name }));
}

export function liveProjectStats(taskLite, phasesLite, today) {
  const byPhase = new Map(phasesLite.map(p => [p.id, { ...p, total: 0, completed: 0, delayed: 0, color: "slate" }]));
  taskLite.forEach(t => {
  ...
  const phaseRows = Array.from(byPhase.values()).map(row => {
    let color = "slate";
...
export function withLiveStats(project, today) {
...
export function aggregateTeamPerformance(allProjectDetails, today) {
