...
import { StatusChip, AchievementBadge, PendingApprovalChip, EmployeeAvatar } from "./common.jsx";
...
export function TaskCard({
  ...
  const [dayOffset, setDayOffset] = useState(task.dayOffset);
  ...
  return (
    <Box sx={{
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="flex-start" gap={2}>
        <Stack direction="row" spacing={1} alignItems="flex-start" sx={{ minWidth: 0 }}>
          ...
          <Box sx={{ minWidth: 0 }}>
            <Stack direction="row" alignItems="center" gap={1} flexWrap="wrap">
              <Typography variant="body2" fontWeight={600} fontSize={14.5}>{task.name}</Typography>
        ...
        {canEdit ? (
          <Select size="small" value={task.status} onChange={(e) => onStatusChange(e.target.value)}
