...
import { OrgSelect, StatusChip, EmployeeAvatar } from "./common.jsx";
...
const TICKET_STATUS_COLOR = { Open: "red", "In Progress": "amber", Resolved: "green", Closed: "slate" };
...
function TicketRow({ ticket, canUpdate, onUpdate }) {
  ...
  return (
    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 1.5, bgcolor: "background.default", border: "1px solid", borderColor: "divider", borderRadius: 2, p: 1.5, flexWrap: "wrap" }}>
      ...
      {canUpdate ? (
        <Select size="small" value={ticket.status} onChange={(e) => onUpdate(ticket.id, { status: e.target.value })} sx={{ minWidth: 140 }}>
...
export function TicketsPanel({ actor, projects, refreshKey }) {
  ...
  const [expanded, setExpanded] = useState({ Raised: true, Completed: false });
  ...
  const grouped = useMemo(() => {
    const g = { Raised: [], Completed: [] };
    tickets.forEach(t => {
      const bucket = BUCKETS.find(b => b.match(t));
      (g[bucket?.key] || g.Raised).push(t);
    });
    return g;
  }, [tickets]);
  ...
  return (
    <Box sx={{ bgcolor: "background.paper", border: "1px solid", borderColor: "divider", borderRadius: 3, p: 2.25, mb: 2.5 }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={1.5} sx={{ mb: 1.75 }}>
        <Box>
          <Typography variant="subtitle1" fontWeight={700}>Reported issues</Typography>
