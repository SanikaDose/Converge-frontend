...
import { buildProjectPhases, buildTasks, summarize, phaseSummaries, projectStatusFromPhases, toTaskLite, toPhasesLite } from "./businessLogic.js";
import { todayISO, addWorkingDays } from "./dateUtils.js";

const store = {
  projects: new Map(), // id -> { id, meta, phases, tasks }
  tickets: [],
  seeded: false,
};
...
function computeIndexRow(project) {
...
export function createProject({ name, type, customer, owner, startDate, endDate }) {
  const id = genId("proj");
  const phases = buildProjectPhases();
  const tasks = buildTasks(startDate, phases);
  const project = { id, meta: { name, type, customer, owner, startDate, endDate, createdAt: todayISO() }, phases, tasks };
  store.projects.set(id, project);
  return project;
}
...
export function createTicket({ title, description, projectId, phase, assignedTo, priority }) {
