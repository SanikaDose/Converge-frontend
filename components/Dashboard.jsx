...
export function Dashboard({ actor, onOpen, refreshKey, onNew }) {
  ...
  const [expanded, setExpanded] = useState({ "In Progress": true, "Completed": true });
  ...
  const projects = useMemo(() => projectsRaw.map(p => withLiveStats(p, today)), [projectsRaw, today]);

  const showMyToggle = role === "Project Manager" || role === "Team Lead" || role === "Team Member";
